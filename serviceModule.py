...
class PoleZero:
    ...
    def __init__(
        self, number: int, real: float, realError: float, imaginary: float, imaginaryError: float
    ) -> None:
        """
...
class PolesZeros:
    ...
    def __init__(
        self, normalizationFactor: float, poles: List[PoleZero], zeros: List[PoleZero]
    ) -> None:
        """
    ...
    try:
        if -90.0 <= float(latitude) <= 90.0:
            ...
            raise argparse.ArgumentTypeError(
                "Not a valid latitude: {0!r}. Value must be between -90.0° and 90.0°".format(
                    latitude
                )
            )
    except:
    ...
    *longitude* is a ``str`` containig a ``float``
    ...
    try:
        if -180.0 <= float(longitude) <= 180.0:
            ...
            raise argparse.ArgumentTypeError(
                "Not a valid longitude: {0!r}. Value must be between -180.0° and 180.0°".format(
                    longitude
                )
            )
    except:
...
def filterTable(table: datascope.Dbptr, columnName: str, tupleCodes: List[Tuple[bool, str]]) -> datascope.Dbptr:
    '''
    ...
    *tupleCodes* is a ``list(tuple)``, where each ``tuple`` is composed by a ``bool`` that indicates
    ...
    if (len(validCodes) > 0):
        ...
        return table.subset(' || '.join(['{0} == "{1}"'.format(columnName, code) for code in list(set(validCodes))]))
    else:
...
def getStationXmlOutput(level: Level, sortedStationInfos: List[SimpleNamespace]) -> str:
    ...
    *sortedStationInfos* is a ``List`` of ``SimpleNamespace`` with the data to generate a StationXML
    ...
    previousStage = (
        ""  # {Station Code}_{Channel+Location Code}_{Calibration Start Epoch}_{Stage Id}
    )
    ...
    for stationInfo in sortedStationInfos:
        ...
        try:
            if previousStation != "{0}_{1}".format(
                stationInfo.stationCode, stationInfo.stationStartDate
            ):
                xmlStation = ET.SubElement(previousXmlNetwork, "Station")
                xmlStation.set("code", stationInfo.stationCode)
                xmlStation.set(
                    "startDate", str(datetime.strptime(str(stationInfo.stationStartDate), "%Y%j"))
                )
                if stationInfo.stationEndDate > 0:
                    xmlStation.set(
                        "endDate", str(datetime.strptime(str(stationInfo.stationEndDate), "%Y%j"))
                    )
        ...
        try:
            if previousChannel != "{0}_{1}_{2}".format(
                stationInfo.stationCode, stationInfo.channelCode, stationInfo.channelStartDate
            ):
                ...
                xmlChannel.set(
                    "startDate", str(datetime.strptime(str(stationInfo.channelStartDate), "%Y%j"))
                )
                if stationInfo.channelEndDate > 0:
                    xmlChannel.set(
                        "endDate", str(datetime.strptime(str(stationInfo.channelEndDate), "%Y%j"))
                    )
                ...
                ET.SubElement(xmlChannel, "SampleRate").text = str(
                    stationInfo.calibrationSampleRate
                )

                xmlChannelSensor = ET.SubElement(xmlChannel, "Sensor")
                ET.SubElement(xmlChannelSensor, "Description").text = (
                    stationInfo.calibrationInstrumentName
                )
                ...
                previousChannel = "{0}_{1}_{2}".format(
                    stationInfo.stationCode, stationInfo.channelCode, stationInfo.channelStartDate
                )
        ...
        try:
            if previousResponse != "{0}_{1}_{2}".format(
                stationInfo.stationCode, stationInfo.calibrationChannel, stationInfo.calibrationTime
            ):
                xmlResponse = ET.SubElement(previousXmlChannel, "Response")

                xmlResponseInstrumentSensitivity = ET.SubElement(
                    xmlResponse, "InstrumentSensitivity"
                )
                ET.SubElement(xmlResponseInstrumentSensitivity, "Value").text = str(
                    1 / float(stationInfo.calibrationCalibration)
                )
                ET.SubElement(xmlResponseInstrumentSensitivity, "Frequency").text = str(
                    stationInfo.calibrationFrequency
                )

                xmlResponseInputUnits = ET.SubElement(
                    xmlResponseInstrumentSensitivity, "InputUnits"
                )
                ET.SubElement(xmlResponseInputUnits, "Name").text = stationInfo.calibrationUnits

                xmlResponseOutputUnits = ET.SubElement(
                    xmlResponseInstrumentSensitivity, "OutputUnits"
                )
                ET.SubElement(xmlResponseOutputUnits, "Name").text = (
                    "Counts"  # TODO Not sure. There is no output units.
                )

                previousResponse = "{0}_{1}_{2}".format(
                    stationInfo.stationCode,
                    stationInfo.calibrationChannel,
                    stationInfo.calibrationTime,
                )
        ...
        try:
            if previousStage != "{0}_{1}_{2}_{3}".format(
                stationInfo.stationCode,
                stationInfo.stageChannel,
                stationInfo.stageTime,
                stationInfo.stageStageId,
            ):
                ...
                ET.SubElement(xmlStageGain, "Frequency").text = str(
                    stationInfo.calibrationFrequency
                )

                previousStage = "{0}_{1}_{2}_{3}".format(
                    stationInfo.stationCode,
                    stationInfo.stageChannel,
                    stationInfo.stageTime,
                    stationInfo.stageStageId,
                ...
                if stationInfo.polesZeros:
                    ...
                    ET.SubElement(xmlPolesZeros, "NormalizationFactor").text = str(
                        stationInfo.polesZeros.normalizationFactor
                    )
                    ...
                    ET.SubElement(xmlDecimation, "InputSampleRate").text = str(
                        stationInfo.decimation.inputSampleRate
                    )
                    ...
                    ET.SubElement(xmlDecimation, "Delay").text = str(
                        stationInfo.stageGainCalibration
                    )  # TODO Not sure
                    ET.SubElement(xmlDecimation, "Correction").text = str(
                        stationInfo.stageGainCalibration
                    )  # TODO Not sure
    ...
    *service* is a ``str`` with the desired service. Valid options are:
    ...
    if domain.startswith("https://") or domain.startswith("http://"):
        ...
        return "http://{0}{1}".format(
            domain, switch.get(service.lower(), "/fdsnws/dataselect/1/query")
        )
...
def getJsonOutput(service: str, groupedRoutingInfos: groupby) -> str:
    ...
    *groupedRoutingInfos* is a list of ``RoutingInfo``, ordered and grouped by their domain
    ...
    for domain, infos in groupedRoutingInfos:
        ...
        routingDictionary = {"name": service, "url": getServiceUrl(domain, service), "params": []}
    ...
    for domain, infos in groupedRoutingInfos:
        ...
        for routingInfo in infos:
            ...
            if len(str(routingInfo.startDate)) > 0 and routingInfo.startDate > 0:
                params["start"] = str(
                    datetime.strptime(str(routingInfo.startDate), "%Y%j").isoformat()
                )
    ...
    *body* is the content of the the POST request
    ...
    for row in rows:
        ...
        if len(rowParts) == 1:
            ...
            postRequests.append(
                PostRequest(
                    rowParts[0], rowParts[1], rowParts[2], rowParts[3], rowParts[4], rowParts[5]
                )
            )
        else:
